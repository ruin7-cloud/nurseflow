...
st.set_page_config(page_title="NurseFlow Pro v3.1", page_icon="🩺", layout="wide")
...
if not api_key:
    with st.sidebar:
        ...
        st.caption("Otomatik key bulunamadı, manuel giriş yapınız.")
    ...
    try:
        ...
        with st.spinner('🤖 Step 1: Processing Input...'):
            ...
            if final_source_type == "audio":
                ...
                audio_prompt = """
                Listen to this clinical note. 
                1. Transcribe main points.
                2. Extract vitals to JSON: 'sbp', 'hr', 'rr', 'spo2', 'temp'.
                ...
                extracted_text_context = result.get("context", "Audio processed")
                if os.path.exists("temp_audio.wav"): os.remove("temp_audio.wav")

            elif final_source_type == "image":
                vision_prompt = """
                Analyze medical monitor. Extract vitals to JSON: 'sbp', 'hr', 'rr', 'spo2', 'temp'.
                Return: {"context": "...", "vitals": {...}}
                """
                response = model.generate_content([vision_prompt, final_content], generation_config={"response_mime_type": "application/json"})
                result = json.loads(response.text)
                vitals = result.get("vitals", {})
                extracted_text_context = result.get("context", "Image processed")

            elif final_source_type == "text":
                text_prompt = f"""
                Extract vitals from text to JSON: 'sbp', 'hr', 'rr', 'spo2', 'temp'.
                Text: {final_content}
                Return: {{"context": "{final_content}", "vitals": {{...}}}} 
                """
        ...
        with st.spinner('🧮 Step 2: Calculating Risk Score...'):
            ...
            if news2_score >= 5:
                bg_color = "#e74c3c"; risk_txt = "HIGH RISK (Emergency)"
            elif news2_score >= 3:
                bg_color = "#f39c12"; risk_txt = "MEDIUM RISK"
            else:
                bg_color = "#27ae60"; risk_txt = "LOW RISK"

        with st.spinner('📝 Step 3: Generating Documentation...'):
            anonymize = "Use 'Patient X'" if privacy_mode else ""
            report_prompt = f"""
            Act as a Senior Nurse. Write ISBAR report.
            Context: {extracted_text_context}
            Vitals: {vitals}
            NEWS2 Score: {news2_score} ({risk_txt})
            Instructions: {anonymize}, professional medical tone.
            """
        ...
        with col_output:
            ...
            <div class="risk-card" style="background-color: {bg_color};">
                <h1 style="margin:0; font-size: 3em;">{news2_score}</h1>
                <h3 style="margin:0;">{risk_txt}</h3>
            </div>
            """, unsafe_allow_html=True)

            res_tab1, res_tab2, res_tab3 = st.tabs(["📄 SBAR Report", "🔢 Vitals Data", "🔍 Context"])
            ...
            with res_tab2:
                ...
                col_v1.metric("BP (Sys)", vitals.get('sbp', '-'))
                col_v2.metric("Heart Rate", vitals.get('hr', '-'))
                col_v1.metric("SpO2", f"{vitals.get('spo2', '-')} %")
                col_v2.metric("Temp", f"{vitals.get('temp', '-')} °C")
                st.caption(f"Score Breakdown: {', '.join(breakdown)}")

            with res_tab3:
                st.info("AI Analysis Context:")
                st.write(extracted_text_context)
